...
namespace LawyerAssistant.Application.Features.BaseDefinitions.FileTypes.Handlers;

public class GetFileTypesListQueryHandler : IRequestHandler<GetFileTypesListQuery, SysResult<List<GenericDTO>>>
{
    private readonly IRepository<FilesTypesModel> _repository;
    public GetFileTypesListQueryHandler(IRepository<FilesTypesModel> repository)
    {
        _repository = repository;
    }
    public async Task<SysResult<List<GenericDTO>>> Handle(GetFileTypesListQuery request, CancellationToken cancellationToken)
    {
